...
class _ChatScreenState extends State<ChatScreen> {
 ...
 void initState() {
  ...
  // Check if 'unreadCounts' field exists, if not, initialize it
  FirebaseFirestore.instance.collection('chats').doc(getChatId()).get().then((docSnapshot) {
    if (!docSnapshot.exists || !docSnapshot.data()!.containsKey('unreadCounts')) {
      // Initialize 'unreadCounts' if it doesn't exist
      FirebaseFirestore.instance.collection('chats').doc(getChatId()).set({
        'unreadCounts': {
          currentUserId: 0,  // Set unread count for current user to 0
          widget.receiverId: 0,  // Set unread count for receiver to 0 (initial state)
        }
